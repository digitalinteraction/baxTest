...
static const char* dbg_file = "serial";
#endif
#include "Debug.h"
...
#ifdef _WIN32
	// Patch for file name bug in windows
	if(infile[0] != '\\')
	{
		sprintf(renamed,"\\\\.\\%s",(char*)infile);
		infile = (const char *)renamed;
	}
#endif
...
int getcSerial(Settings_t* settings)
{
	int res, ret;
	unsigned char read;
	if(settings->fd < 0) return -1;
	res = readport(settings->fd, &read, 1, 0);
	if(res == 1) ret = (unsigned int)read;
	else ret = -1;
	return ret;
}
...
#if defined(_WIN32) && defined(WIN_HANDLE)
	if (WriteFile((HANDLE)settings->fd, &b, 1, &written, NULL)) { ret = TRUE; }
	else { ret = -1; }
#else
	if(write(settings->fd, &b, 1) == 1) ret = TRUE;
	else ret = -1;
#endif
	return ret;
}
